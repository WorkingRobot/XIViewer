...
const outDir = join(root, "smoke", "glasses_probe5");
...
async function main() {
    ...
    const profile = mkdtempSync(join(tmpdir(), "xiviewer-glasses-wait-"));
    ...
    cdp.on("Runtime.consoleAPICalled", (p: any) => {
        ...
        if (line.includes("character") || line.includes("mdl")) console.log(`   | ${line}`);
    ...
    cdp.on("Network.responseReceived", (p: any) => {
        if (seen.has(p.requestId)) console.log(`   << RESP ${p.response.status} ${p.response.url}`);
    });
    ...
    try {
        ...
        console.log("   waiting 20s for redress...");
        await sleep(20000);
        await shot(cdp, "00-after20s");
        // Zoom to the face.
        await wheel(cdp, 950, 300, -60, 10);
        await sleep(1500);
        await shot(cdp, "01-zoomed");
        console.log("SUCCESS");
    } finally {
        cdp.close();
        child.kill();
